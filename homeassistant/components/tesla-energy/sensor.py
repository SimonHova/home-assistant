...
def _check_sensor_schema(conf):
    ...
    try:
        ...
        valid = [s.name for s in powerwall_site.Sensors()]
...
async def async_setup_platform(
    ...
    sensor_def = powerwall_site.Sensors()

    # Sensor from the custom config
    sensor_def.add([powerwall_site.Sensor(o[CONF_KEY], n, o[CONF_UNIT], o[CONF_FACTOR],
                                 o.get(CONF_PATH))
    ...
    sma = powerwall_site.SMA(session, url, config[CONF_PASSWORD], group=grp)
